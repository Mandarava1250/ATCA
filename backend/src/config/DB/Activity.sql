...
-- 用户打卡统计视图
GO
CREATE OR ALTER VIEW [vw_user_checkin_stats] AS
SELECT
    [user_id],
...
-- 存储过程：用户打卡
GO
CREATE OR ALTER PROCEDURE [dbo].[sp_user_checkin]
    @user_id INT,
    @device_type VARCHAR(20) = NULL,
    @device_info NVARCHAR(255) = NULL,
...
-- 存储过程：获取用户打卡记录
GO
CREATE OR ALTER PROCEDURE [dbo].[sp_get_user_checkins]
    @user_id INT,
    @page INT = 1,
    @limit INT = 30
AS
...
-- 存储过程：获取用户打卡统计
GO
CREATE OR ALTER PROCEDURE [dbo].[sp_get_user_checkin_stats]
    @user_id INT
AS
BEGIN
    SET NOCOUNT ON;
...
-- 存储过程：检查今日是否已打卡
GO
CREATE OR ALTER PROCEDURE [dbo].[sp_check_today_checkin]
    @user_id INT
AS
BEGIN
    SET NOCOUNT ON;
...
-- 存储过程：获取打卡日历数据
GO
CREATE OR ALTER PROCEDURE [dbo].[sp_get_checkin_calendar]
    @user_id INT,
    @year INT,
    @month INT
AS
