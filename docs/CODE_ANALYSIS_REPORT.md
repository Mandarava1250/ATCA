...
**华夏营造（ATCA - Ancient Traditional Chinese Architecture）** 是一个以中国传统古建筑文化为核心的全栈 Web 应用平台。项目融合了**文化科普教育**、**3D 交互建模**、**知识答题闯关**、**多语言国际化**四大核心功能模块，旨在通过数字化手段传承和推广中国传统建筑文化。
...
ATCA（华夏营造）是一个功能丰富、架构清晰的全栈文化教育平台。项目采用前后端分离架构，通过 RESTful API 和 WebSocket 实现数据交互，使用 Three.js 构建了具有物理引擎和榫卯吸附系统的 3D 建模工坊。代码整体质量较高，TypeScript 类型覆盖完整，安全防护（JWT双令牌、SQL参数化、输入校验）到位。
