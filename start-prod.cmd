@echo off
setlocal enabledelayedexpansion

:: ============================================
:: ATCA 生产环境自动化部署脚本
:: ============================================

set "PROJECT_ROOT=%~dp0"
set "BACKEND_DIR=%PROJECT_ROOT%backend"
set "FRONTEND_DIR=%PROJECT_ROOT%frontend"
set "NGINX_DIR=C:\nginx"
set "LOG_DIR=%PROJECT_ROOT%logs"
set "BACKEND_LOG=%LOG_DIR%\backend.log"
set "ERROR_LOG=%LOG_DIR%\error.log"

set "BACKEND_PORT=3000"
set "NGINX_PORT=80"

:: 创建日志目录
if not exist "%LOG_DIR%" (
    mkdir "%LOG_DIR%"
)

:: ============================================
:: 清除控制台颜色和日志
:: ============================================
title ATCA 生产环境部署
echo.
echo ============================================
echo   ATCA 生产环境自动化部署脚本
echo ============================================
echo.

:: ============================================
:: 步骤1: 检查 Node.js
:: ============================================
echo [步骤 1/8] 检查 Node.js 环境...
node --version >nul 2>&1
if %errorlevel% neq 0 (
    echo   [错误] 未找到 Node.js，请先安装 Node.js v18 或更高版本
    echo   下载地址: https://nodejs.org/
    pause
    exit /b 1
)
for /f "delims=" %%i in ('node --version') do set "NODE_VERSION=%%i"
echo   [OK] Node.js !NODE_VERSION!

:: ============================================
:: 步骤2: 检查 npm
:: ============================================
echo.
echo [步骤 2/8] 检查 npm 环境...
npm --version >nul 2>&1
if %errorlevel% neq 0 (
    echo   [错误] 未找到 npm
    pause
    exit /b 1
)
for /f "delims=" %%i in ('npm --version') do set "NPM_VERSION=%%i"
echo   [OK] npm !NPM_VERSION!

:: ============================================
:: 步骤3: 安装依赖
:: ============================================
echo.
echo [步骤 3/8] 安装项目依赖...

:: 根目录依赖
if not exist "%PROJECT_ROOT%node_modules" (
    echo   正在安装根目录依赖...
    cd /d "%PROJECT_ROOT%"
    call npm install
    if !errorlevel! neq 0 (
        echo   [错误] 根目录依赖安装失败
        pause
        exit /b 1
    )
)

:: 前端依赖
if not exist "%FRONTEND_DIR%\node_modules" (
    echo   正在安装前端依赖...
    cd /d "%FRONTEND_DIR%"
    call npm install
    if !errorlevel! neq 0 (
        echo   [错误] 前端依赖安装失败
        pause
        exit /b 1
    )
)

:: 后端依赖
if not exist "%BACKEND_DIR%\node_modules" (
    echo   正在安装后端依赖...
    cd /d "%BACKEND_DIR%"
    call npm install
    if !errorlevel! neq 0 (
        echo   [错误] 后端依赖安装失败
        pause
        exit /b 1
    )
)
echo   [OK] 所有依赖安装完成

:: ============================================
:: 步骤4: 端口检测
:: ============================================
echo.
echo [步骤 4/8] 检测端口占用情况...

:: 检测后端端口
netstat -ano | findstr :%BACKEND_PORT% >nul 2>&1
if %errorlevel% equ 0 (
    echo   [警告] 端口 %BACKEND_PORT% 已被占用，正在尝试终止进程...
    for /f "tokens=5" %%a in ('netstat -ano ^| findstr :%BACKEND_PORT%') do (
        taskkill /F /PID %%a >nul 2>&1
    )
    timeout /t 2 /nobreak >nul
)

:: 检测 nginx 端口
netstat -ano | findstr :%NGINX_PORT% >nul 2>&1
if %errorlevel% equ 0 (
    echo   [警告] 端口 %NGINX_PORT% 已被占用，正在尝试终止进程...
    for /f "tokens=5" %%a in ('netstat -ano ^| findstr :%NGINX_PORT%') do (
        taskkill /F /PID %%a >nul 2>&1
    )
    timeout /t 2 /nobreak >nul
)
echo   [OK] 端口检测完成

:: ============================================
:: 步骤5: 构建项目
:: ============================================
echo.
echo [步骤 5/8] 构建项目...

:: 构建前端
echo   正在构建前端...
cd /d "%FRONTEND_DIR%"
call npm run build
if !errorlevel! neq 0 (
    echo   [错误] 前端构建失败
    pause
    exit /b 1
)
echo   [OK] 前端构建完成

:: 构建后端
echo   正在构建后端...
cd /d "%BACKEND_DIR%"
call npm run build
if !errorlevel! neq 0 (
    echo   [错误] 后端构建失败
    pause
    exit /b 1
)
echo   [OK] 后端构建完成

:: ============================================
:: 步骤6: 检查 nginx
:: ============================================
echo.
echo [步骤 6/8] 检查 nginx...

if not exist "%NGINX_DIR%\nginx.exe" (
    echo   [错误] 未找到 nginx，请确保 nginx 已安装在 C:\nginx
    echo   下载地址: https://nginx.org/en/download.html
    pause
    exit /b 1
)
echo   [OK] nginx 已找到

:: 复制 nginx 配置
if exist "%FRONTEND_DIR%\nginx.conf" (
    echo   正在复制 nginx 配置文件...
    copy /Y "%FRONTEND_DIR%\nginx.conf" "%NGINX_DIR%\conf\nginx.conf" >nul
    echo   [OK] nginx 配置已更新
)

:: ============================================
:: 步骤7: 启动后端服务
:: ============================================
echo.
echo [步骤 7/8] 启动后端服务...

:: 终止已有的后端进程
wmic process where "name='node.exe' and commandline like '%%dist%%main.js%%'" call terminate >nul 2>&1
timeout /t 1 /nobreak >nul

:: 设置环境变量并启动后端
cd /d "%BACKEND_DIR%"
set "NODE_ENV=production"

:: 启动后端并记录日志
start /B cmd /c "title ATCA Backend Server && node dist/main.js >> \"%BACKEND_LOG%\" 2>&1"

timeout /t 3 /nobreak >nul

:: 检查后端是否成功启动
netstat -ano | findstr :%BACKEND_PORT% >nul 2>&1
if %errorlevel% neq 0 (
    echo   [错误] 后端服务启动失败，请查看日志: %BACKEND_LOG%
    pause
    exit /b 1
)
echo   [OK] 后端服务已启动 (端口 %BACKEND_PORT%)

:: ============================================
:: 步骤8: 启动 nginx
:: ============================================
echo.
echo [步骤 8/8] 启动 nginx...

:: 停止已有的 nginx
cd /d "%NGINX_DIR%"
nginx -s stop >nul 2>&1
timeout /t 1 /nobreak >nul

:: 启动 nginx
start /B cmd /c "title ATCA Nginx Server && nginx"

timeout /t 2 /nobreak >nul

:: 检查 nginx 是否成功启动
netstat -ano | findstr :%NGINX_PORT% | findstr LISTENING >nul 2>&1
if %errorlevel% neq 0 (
    echo   [错误] nginx 启动失败，请查看错误日志
    pause
    exit /b 1
)
echo   [OK] nginx 已启动 (端口 %NGINX_PORT%)

:: ============================================
:: 部署完成
:: ============================================
echo.
echo ============================================
echo   部署完成！
echo ============================================
echo.
echo   服务状态:
echo   - 后端 API:  http://127.0.0.1:%BACKEND_PORT%/api/v1
echo   - 后端健康:  http://127.0.0.1:%BACKEND_PORT%/health
echo   - 前端页面:  http://localhost
echo   - API 文档:  http://127.0.0.1:%BACKEND_PORT%/api-docs
echo.
echo   日志文件:
echo   - 后端日志:  %BACKEND_LOG%
echo   - 错误日志:  %NGINX_DIR%\logs\error.log
echo.
echo   按任意键打开日志目录...
pause >nul
explorer "%LOG_DIR%"

endlocal
exit /b 0
